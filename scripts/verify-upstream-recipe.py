...
EXPECTED_SCRIPT = "source_tag=$(cd upstream && CI_FORCE_CLEAN=1 bin/root-tag); exec docker buildx build --file upstream/web/Dockerfile --platform linux/amd64,linux/arm64 --build-arg LINKERD_VERSION=${source_tag} --tag linkerd2-web-benchmark:local upstream"
...
def main() -> int:
    try:
        ...
        return 1
    print("Verified Linkerd web image plan against bin/docker-build-web.")
    return 0
